...
install.packages("rvest") # 정적 크롤링시에 필요
install.packages("XML") # 정적 크롤링시에 필요
install.packages("httr") # 정적 크롤링시에 필요
...
text <- read_html(url) #서버에 요청하여 페이지 끌어옴
text

nodes <- html_nodes(text, "h1") # 태그 선택자, dom 객체들을 찾아옴
nodes
